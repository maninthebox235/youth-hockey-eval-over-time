...
def add_reset_fields():
    ...
    with app.app_context():
        try:
            db.engine.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS reset_token VARCHAR(256)")
            db.engine.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS reset_token_expiry TIMESTAMP")
            ...
            try:
                db.session.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS reset_token VARCHAR(256)"))
