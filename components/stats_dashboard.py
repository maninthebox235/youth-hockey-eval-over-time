...
def display_age_group_stats(players_df):
    ...
    age_groups = players_df.groupby('age_group').agg({
        'player_id': 'count',
        'skating_speed': 'mean',
    ...
    age_groups.columns = ['Players', 'Avg Skating Speed', 
                         'Avg Shooting Accuracy', 'Avg Games Played']
    
    st.dataframe(age_groups)
    
    fig = px.bar(players_df, x='age_group', y=['skating_speed', 'shooting_accuracy'],
...
def display_player_rankings(players_df):
    ...
    with col2:
        top_shooting = players_df.nlargest(5, 'shooting_accuracy')[['name', 'shooting_accuracy']]
