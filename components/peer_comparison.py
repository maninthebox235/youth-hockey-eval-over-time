...
def display_peer_comparison(player_data):
    ...
    if peer_df.empty:
        st.info(f"No peers found for comparison (Age: {player_age}, Position: {player_position})")
        return
    ...
    position_metrics = {
        'Forward': ['skating_speed', 'shooting_accuracy', 'puck_control', 'passing_accuracy', 'hockey_sense'],
        'Defense': ['skating_speed', 'backward_skating', 'gap_control', 'shot_blocking', 'breakout_passes'],
        'Goalie': ['save_percentage', 'positioning', 'rebound_control', 'recovery', 'save_technique']
    }
    
    default_metrics = position_metrics.get(player_position, ['skating_speed', 'shooting_accuracy'])
    available_metrics = [col for col in peer_df.columns if col not in ['peer_id', 'age', 'position', 'games_played']]
        ...
        if selected_metrics:
            ...
            if dist_metric:
                ...
                if player_value is not None and dist_metric in peer_df.columns:
                    ...
                    with col2:
                        st.metric("Peer Average", f"{peer_mean:.1f}", 
            ...
            if weaknesses:
                for metric, percentile in weaknesses:
                    ...
                    if metric == 'skating_speed':
                        st.info("**Training Focus:** Edge work, stride efficiency, and explosive starts")
                    elif metric == 'shooting_accuracy':
                        st.info("**Training Focus:** Shot repetition, proper weight transfer, target practice")
                    elif metric == 'puck_control':
                        st.info("**Training Focus:** Stickhandling drills, tight space control, head-up practice")
                    elif metric == 'passing_accuracy':
                        st.info("**Training Focus:** Pass reception, proper technique, moving targets")
                    else:
                        st.info(f"**Training Focus:** Consistent practice of {metric_name.lower()} fundamentals")
        ...
        with col2:
            ...
            if strengths:
                for metric, percentile in strengths:
                    ...
                    if percentile >= 90:
                        st.success("**Focus:** Continue to develop this elite skill and teach others")
                    elif percentile >= 75:
                        st.success("**Focus:** Refine this strength with advanced techniques")
                    else:
                        st.success("**Focus:** Build on this strength in game situations")
    ...
    try:
        ...
        if selected_metric:
            ...
            if len(dates) > 1:
                ...
                metric_benchmark = get_age_appropriate_benchmark(player_age, selected_metric)
                if metric_benchmark:
                ...
                for date in dates:
                    ...
                    peers_df = get_peer_group_data(player_age, position, selected_metric)
                ...
                if len(dates) >= 2:
                    ...
                    with col2:
                        st.metric("Current Value", f"{latest_value:.1f}", delta=f"{improvement:.1f}")
                    with col3:
