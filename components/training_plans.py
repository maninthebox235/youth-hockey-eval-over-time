    ...
    def generate_personalized_plan(self, player_data):
        ...
        for skill, value in needs:
            ...
            for drill in skill_drills:
                plan["drills"].append({"skill": skill, "skill_rating": value, **drill})
...
def display_drill_details(drill):
    ...
    if st.button(
        "Add to Training Schedule", key=f"add_{drill['name'].replace(' ', '_')}"
    ):
                    ...
                    gauge={
                        ...
                        "bar": {
                            "color": (
                                "red"
                                if value < 2.5
                                else "yellow" if value < 3.5 else "green"
                            )
                        },
                        "steps": [
    ...
    if age_plan <= len(days):
        ...
        for i, day in enumerate(schedule_days):
            with day_tabs[i]:
                ...
                if day_drills:
                    st.write(
                        f"**Session Duration:** {plan['session_duration']} minutes"
                    )

                    for drill in day_drills:
                        with st.expander(f"{drill['name']} ({drill['duration']})"):
                            st.write(
                                f"**Focus Area:** {drill['skill'].replace('_', ' ').title()}"
                            )
                            st.write(f"**Difficulty:** {drill['difficulty']}")
...
def display_training_plan_interface(player_id, player_data):
    ...
    if not is_premium:
        ...
        with col2:
            ...
            st.image(
                ...
                caption="Training Plans Preview",
    ...
    player_name = "Player"
    if (
        isinstance(player_data, pd.DataFrame)
        and "name" in player_data.columns
        and not player_data.empty
    ):
    ...
    tabs = st.tabs(
        ["Focus Areas", "Weekly Schedule", "Training Library", "Progress Tracking"]
    )
    ...
    with tabs[0]:
        ...
        for skill in plan["focus_areas"]:
            ...
            if skill == "skating_speed":
                ...
                st.info(
                    "Develop confident inside and outside edge work for better control and agility"
                )
