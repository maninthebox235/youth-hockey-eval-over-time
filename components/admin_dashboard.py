...
def display_player_details(player_id):
    ...
    with tab3:
        ...
        feedback = (
            CoachFeedback.query.filter_by(player_id=player.id)
        ...
        if feedback:
            for entry in feedback:
                with st.expander(
                    f"{entry.date.strftime('%Y-%m-%d')} - {entry.coach_name}"
                ):
...
def display_team_details(team_id):
    ...
    st.write(
        f"Games: {team.games_played or 0} | Wins: {team.wins or 0} | Losses: {team.losses or 0}"
    )
...
def display_user_management():
    ...
    with st.form("admin_status_form"):
        ...
        if submitted:
            ...
            if success:
                st.success(
                    f"User {user_id} admin status updated to: {'Admin' if make_admin else 'Regular User'}"
                )
                st.rerun()
            else:
...
def get_analytics_data():
    ...
    users_last_month = sum(
        1 for user in users if user.created_at and user.created_at >= month_ago
    )
    users_last_week = sum(
        1 for user in users if user.created_at and user.created_at >= week_ago
    )
...
def display_admin_dashboard():
    ...
    if "user" not in st.session_state or not st.session_state.user.get(
        "is_admin", False
    ):
