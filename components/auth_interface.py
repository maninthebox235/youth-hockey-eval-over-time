    ...
    with st.form("login_form"):
        ...
        with col1:
            remember_me = st.checkbox(
                "Remember Me", value=True, help="Keep me logged in for 30 days"
            )
        with col2:
        ...
        if submitted:
            ...
            try:
                ...
                try:
                    ...
                    expiration = (
                        30 * 24 * 3600 if remember_me else 24 * 3600
                    )  # 30 days or 1 day
...
def display_auth_interface():
    ...
    if not st.session_state.get("user"):
        ...
        with st.sidebar:
            ...
            is_admin = (
                st.session_state.user.get("is_admin", False)
                if isinstance(st.session_state.user, dict)
                else False
            )
            is_premium = (
                st.session_state.user.get("is_premium", False)
                if isinstance(st.session_state.user, dict)
                else False
            )
            ...
            with st.container():
                ...
                with col1:
                    ...
                    # Show session type
                    if (
                        "remember_me" in st.session_state
                        and st.session_state.remember_me
                    ):
