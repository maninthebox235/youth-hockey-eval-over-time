...
class OffIceTraining:
    def __init__(self):
        ...
        self.age_based_plans = {
            (6, 8): {
                ...
                "schedule": [
                    "15-20 min stickhandling",
                    "15 min bodyweight activities",
                    "15 min coordination games",
                ],
            ...
            (9, 11): {
                ...
                "schedule": [
                    "20 min stickhandling",
                    "20 min bodyweight training",
                    "20 min shooting or balance",
                ],
            ...
            (12, 14): {
                ...
                "schedule": [
                    "25 min stickhandling/shooting",
                    "25 min strength training",
                    "15 min agility/speed",
                ],
    ...
    def record_training_session(
        self, player_id, training_date, training_type, duration, activities, notes
    ):
        """
        ...
        try:
            ...
            for entry in history_entries:
                if entry.notes and "OFF-ICE TRAINING:" in entry.notes:
                    training_type = (
                        entry.notes.split("OFF-ICE TRAINING:")[1].split("\n")[0].strip()
                    )
                    ...
                    if "Activities:" in entry.notes:
                        activities = (
                            entry.notes.split("Activities:")[1].split("\n")[0].strip()
                        )
                    if "Duration:" in entry.notes:
                        duration = (
                            entry.notes.split("Duration:")[1].split("\n")[0].strip()
                        )
            ...
            if off_ice_entries:
                ...
                for i, entry in enumerate(off_ice_entries):
                    with st.expander(
                        f"{entry['date']} - {entry['type']}", expanded=i == 0
                    ):
    ...
    def display_training_form(self, player_id, player_name):
        ...
        with st.form("off_ice_training_form"):
            ...
            if training_category:
                ...
                training_subcategory = st.selectbox(
                    "Training Subcategory", subcategories
                )

                # Activities based on subcategory
                if training_subcategory:
                    activity_options = self.training_categories[training_category][
