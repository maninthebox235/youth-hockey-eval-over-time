...
def validate_roster_data(df):
    ...
    if not errors and len(df) > 0:
        ...
        invalid_pos = [
            ...
            if pos not in valid_positions and pd.notna(pos)
        ]
        if invalid_pos:
            errors.append(
                f"Invalid positions found: {', '.join(invalid_pos)}. Must be one of {', '.join(valid_positions)}"
            )

        if "jersey_number" in df.columns:
            if not all(
                isinstance(num, (int, float)) or pd.isna(num)
                for num in df["jersey_number"]
            ):
        ...
        if "team_name" in df.columns and not all(pd.isna(df["team_name"])):
            ...
            for team_name, players in team_groups:
                ...
                if not Team.query.filter_by(name=team_name).first():
                    age_group = (
                        players["age_group"].iloc[0]
                        if "age_group" in players.columns
                        else "Unknown"
                    )
                    new_team = Team(
                        name=team_name,
...
def add_player_from_data(player_data, team_id=None, user_id=None):
    ...
    new_player = Player(
        ...
        age_group=(
            player_data["age_group"]
            if "age_group" in player_data
            else get_age_group(int(player_data["age"]))
        ),
    ...
    with st.form("add_player_form"):
        ...
        with col2:
            jersey_number = st.number_input(
                "Jersey Number", min_value=0, max_value=99, value=0
            )
...
def display_upload_roster_form():
    ...
    st.markdown(
        get_download_link(
            template_df, "player_roster_template.csv", "Download Roster Template"
        ),
...
def display_create_team_form():
    ...
    with st.form("create_team_form"):
        ...
        if submitted and team_name:
            try:
                new_team = Team(
                    name=team_name, age_group=age_group, created_date=datetime.utcnow()
                )
    ...
    if teams:
        ...
        for team in teams:
            with st.expander(f"{team.name} ({team.age_group})"):
                ...
                if user_id:
                    players = (
                        Player.query.join(TeamMembership)
                        .filter(
                            TeamMembership.team_id == team.id, Player.user_id == user_id
                        )
                        .all()
                    )
                else:
                ...
                if players:
                    ...
                    for player in players:
                        memberships = player.memberships.filter_by(
                            team_id=team.id
                        ).first()
                        position = (
                            memberships.position_in_team
                            if memberships
                        ...
                        data.append(
                            {
                                ...
                                "Jersey": (
                                    ...
                                    if hasattr(player, "jersey_number")
                                    else "-"
                                ),
                            }
                        )
