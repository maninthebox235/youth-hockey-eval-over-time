        ...
        if analysis_type in self.feedback_templates:
            ...
            for metric, score in metrics.items():
                if metric in templates:
                    if score >= 3.5:
                        feedback.append(
                            f"**{metric.replace('_', ' ').title()}**: {templates[metric]['good']}"
                        )
                    else:
                        feedback.append(
                            f"**{metric.replace('_', ' ').title()}**: {templates[metric]['needs_work']}"
                        )
                else:
                    # Generic feedback if no template exists
                    if score >= 3.5:
                        feedback.append(
                            f"**{metric.replace('_', ' ').title()}**: Good performance, continue refining technique."
                        )
                    else:
                        feedback.append(
                            f"**{metric.replace('_', ' ').title()}**: Needs improvement. Focus on proper technique."
                        )
...
def display_video_upload_form(position):
    ...
    uploaded_file = st.file_uploader(
        "Upload Video (MP4, MOV, or MKV)", type=["mp4", "mov", "mkv"]
    )
    ...
    with st.form("video_analysis_form"):
        ...
        if analyze_button and uploaded_file is not None:
            ...
            with st.spinner("Analyzing video... This may take a minute..."):
                ...
                analysis_results = analyzer.perform_mock_analysis(
                    video_bytes, analysis_type
                )
...
def display_video_analysis_results():
                    ...
                    gauge={
                        ...
                        "bar": {
                            "color": (
                                "red"
                                if score < 2.5
                                else "yellow" if score < 3.5 else "green"
                            )
                        },
                        "steps": [
    ...
    recommended_drills = analyzer.get_improvement_drills(
        results["analysis_type"], results["metrics"]
    )
...
def display_video_analysis_interface(player_id, player_data):
    ...
    if not is_premium:
        ...
        with col2:
            ...
            st.image(
                ...
                caption="Video Analysis Preview",
                use_container_width=True,
            )
        return
    ...
    st.title("Video Analysis")
    st.write(
        "Upload videos of your hockey skills for AI-assisted analysis and feedback"
    )
