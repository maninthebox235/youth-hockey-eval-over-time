        ...
        data.append(
            {
                ...
                "shooting_accuracy": min(
                    5.0, shooting_base + np.random.normal(0, 0.25)
                ),
                "puck_control": min(5.0, 2.5 + (i * 0.15) + np.random.normal(0, 0.15)),
                "passing_accuracy": min(
                    5.0, 3.0 + (i * 0.08) + np.random.normal(0, 0.2)
                ),
...
def generate_progress_chart(player_history, metric="skating_speed"):
    ...
    fig = px.line(player_history, x="date", y=metric, title=title, markers=True)
...
def display_feature_showcase():
    ...
    with showcase_tabs[1]:
        ...
        with dev_col2:
            shooting_chart = generate_progress_chart(
                player_history, "shooting_accuracy"
            )
    ...
    with showcase_tabs[2]:
        ...
        st.write(
            "Compare player performance to peers in the same age group and position"
        )
    ...
    with showcase_tabs[3]:
        ...
        with pos_col2:
            ...
            goalie_metrics = {
                ...
                / 20,  # Scale to 1-5 for radar chart
            }

            goalie_radar_fig = go.Figure()

            goalie_radar_fig.add_trace(
                go.Scatterpolar(
