...
def test_email_configuration(mail, test_email=None):
    ...
    try:
        ...
        if not mail.server or not mail.username or not mail.password:
            logging.error(
                f"Missing mail configuration: Server: {bool(mail.server)}, Username: {bool(mail.username)}, Password: {bool(mail.password)}"
            )
            return {
                "success": False,
        ...
        if "getaddrinfo failed" in error_details:
            ...
            suggestion = (
                "SSL/TLS error. Your email provider may require different SSL settings."
            )
        elif "Timeout" in error_details:
            suggestion = (
                "Connection timed out. Check your network settings or try again later."
            )
