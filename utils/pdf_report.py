...
class PDFReportGenerator:
    ...
    def generate_player_report(self, player_id):
        ...
        report_elements.append(Paragraph(f"Player Assessment Report", self.custom_styles['Title']))
        report_elements.append(Spacer(1, 0.1*inch))
        report_elements.append(Paragraph(f"Player: {player.name}", self.custom_styles['Heading1']))
        ...
        report_elements.append(Paragraph("Skill Assessment", self.custom_styles['Heading1']))
        report_elements.append(Spacer(1, 0.1*inch))
        ...
        for section_name, metrics in skill_sections.items():
            ...
            if len(skill_data) > 1:  # Only add table if there are skills
                skill_table = Table(skill_data, colWidths=[1.5*inch, 0.8*inch, 3*inch])
        ...
        # Add Coach Feedback
        feedback_list = CoachFeedback.query.filter_by(player_id=player_id).order_by(CoachFeedback.date.desc()).limit(3).all()
        if feedback_list:
            report_elements.append(Paragraph("Recent Coach Feedback", self.custom_styles['Heading1']))
            report_elements.append(Spacer(1, 0.1*inch))
            ...
            for i, feedback in enumerate(feedback_list):
                ...
                report_elements.append(Paragraph(feedback.feedback_text, self.custom_styles['BodyText']))
        ...
        if history and len(history) > 1:
            report_elements.append(Paragraph("Development Progress", self.custom_styles['Heading1']))
            report_elements.append(Spacer(1, 0.1*inch))
            
            # Get key metrics to track over time based on position
            key_metrics = self._get_key_metrics_for_position(player.position)
            progress_data = [['Date'] + [m.replace('_', ' ').title() for m in key_metrics]]
    ...
    def _get_skill_sections(self, player):
        ...
        if player.position.lower() in ['forward', 'center', 'wing', 'left wing', 'right wing']:
    ...
    def _get_key_metrics_for_position(self, position):
        ...
        if position.lower() in ['forward', 'center', 'wing', 'left wing', 'right wing']:
            ...
            return ['positioning', 'save_technique', 'rebound_control', 'reaction_time', 'recovery']
